...
class HyperparameterOptimizer:
    ...
    def objective(self, trial):
        """Objective function for hyperparameter optimization with refined search space."""
        # Primary model architecture (focusing on hybrid with exploration)
        model_type = trial.suggest_categorical('model_type', ['hybrid', 'transformer', 'lstm'], 
                                            weights=[0.6, 0.2, 0.2])
        
        # Learning rate (narrow range around optimal value)
        learning_rate = trial.suggest_float('learning_rate', 0.0001, 0.0008, log=True)
        
        # Batch size (focused around optimal)
        batch_size = trial.suggest_categorical('batch_size', [24, 32, 48, 64])
        
        # Dropout (centered around 0.45)
        dropout_rate = trial.suggest_float('dropout_rate', 0.35, 0.55, step=0.05)
        
        # Architecture parameters (narrower ranges around optimal values)
        num_heads = trial.suggest_categorical('num_heads', [2, 3, 4, 6])
        ff_dim = trial.suggest_categorical('ff_dim', [224, 256, 320, 384])
        embed_dim = trial.suggest_categorical('embed_dim', [96, 112, 128, 144])
        conv_filters = trial.suggest_int('conv_filters', 8, 32, step=8)
        
        # Transformer blocks (leaning toward 3-4 blocks)
        num_transformer_blocks = trial.suggest_int('num_transformer_blocks', 2, 4)
        
        # Favoring AdamW but still exploring others
        optimizer = trial.suggest_categorical('optimizer', ['adamw', 'adam', 'rmsprop'], 
                                            weights=[0.7, 0.2, 0.1])
        
        # Sequence length (focusing on longer sequences)
        sequence_length = trial.suggest_categorical('sequence_length', [24, 28, 32, 36, 40])
        
        # Regularization (narrower range around optimal)
        l2_regularization = trial.suggest_float('l2_regularization', 1e-5, 1e-4, log=True)
        
        # Binary parameters strongly favoring the optimal values
        use_gru = trial.suggest_categorical('use_gru', [True, False], weights=[0.9, 0.1])
        use_residual = trial.suggest_categorical('use_residual', [True, False], weights=[0.9, 0.1])
        use_layer_scaling = trial.suggest_categorical('use_layer_scaling', [True, False], weights=[0.9, 0.1])
        
        # Early stopping delta (around optimal)
        min_delta = trial.suggest_float('min_delta', 0.0002, 0.0008, log=True)
        
        # Model component usage - key insight from previous optimization
        # Exploring pattern model dominance but also ensemble combinations
        model_config = trial.suggest_categorical('model_config', 
                                            ['pattern_only', 'pattern_dominant', 'transformer_dominant', 'balanced'],
                                            weights=[0.4, 0.3, 0.15, 0.15])
        
        # Set model usage flags based on configuration
        if model_config == 'pattern_only':
            use_pattern_model = True
            use_transformer_model = False
            use_frequency_model = False
            ensemble_weights = {'pattern': 1.0}
        elif model_config == 'pattern_dominant':
            use_pattern_model = True
            use_transformer_model = trial.suggest_categorical('use_transformer_model', [True, False])
            use_frequency_model = trial.suggest_categorical('use_frequency_model', [True, False])
            # Pattern-dominant weights
            if use_transformer_model and use_frequency_model:
                ensemble_weights = {'pattern': 0.6, 'transformer': 0.2, 'frequency': 0.2}
            elif use_transformer_model:
                ensemble_weights = {'pattern': 0.7, 'transformer': 0.3}
            elif use_frequency_model:
                ensemble_weights = {'pattern': 0.7, 'frequency': 0.3}
            else:
                ensemble_weights = {'pattern': 1.0}
        elif model_config == 'transformer_dominant':
            use_pattern_model = True
            use_transformer_model = True
            use_frequency_model = trial.suggest_categorical('use_frequency_model', [True, False])
            # Transformer-dominant weights
            if use_frequency_model:
                ensemble_weights = {'transformer': 0.6, 'pattern': 0.25, 'frequency': 0.15}
            else:
                ensemble_weights = {'transformer': 0.7, 'pattern': 0.3}
        else:  # balanced
            use_pattern_model = True
            use_transformer_model = True
            use_frequency_model = True
            ensemble_weights = {'pattern': 0.34, 'transformer': 0.33, 'frequency': 0.33}
        
        # Model-specific parameters
        if model_type == 'lstm' or model_type == 'hybrid':
            if model_type == 'lstm':
                lstm_units = trial.suggest_categorical('lstm_units_lstm', [24, 32, 48, 64])
            else:  # hybrid
                lstm_units = trial.suggest_categorical('lstm_units_hybrid', [16, 24, 32, 48])
                gru_units = trial.suggest_categorical('gru_units', [24, 32, 48, 64])
        ...
        params = {
            ...
            'use_layer_scaling': use_layer_scaling,
            'min_delta': min_delta,
            'use_frequency_model': use_frequency_model,
